...
PYTHON_SCRIPT="experiments/sparse_v3.py" # Make sure this filename is correct
...
echo "Starting SPARSE experiments. Results will be saved in ${OUTPUT_DIR}"
echo "=================================================="
...
do
    ...
    sbatch << EOF
#!/bin/bash -l
#SBATCH --job-name=${MODEL_NAME}        # Job name (unique for each model)
#SBATCH --partition=gpu_short           # Use the GPU short partition
#SBATCH --array=0-${TOTAL_JOBS}         # Job array indices
#SBATCH --time=4:00:00                  # Max runtime (gpu_short allows up to 4 hours)
#SBATCH --gres=gpu:1                    # Request one GPU, required for this partition
...
#SBATCH --mem=16G                       # Memory per node (gpu nodes often have more RAM)
...
echo "All sparse model jobs have been submitted."