...
class TPR(nn.Module):
    def __init__(
        ...
        # __init__ 内
        # self.log_noisescale = nn.Parameter(torch.log(noisescale)) # 削除
        self.register_buffer('noisescale', noisescale) # 追加
    ...
    def _update_noisescale(self, local_params):
        alpha_lambda, beta_lambda = local_params
        E_lambda = alpha_lambda / beta_lambda.clamp(min=EPSILON)
        
        # E_sq_err は _e_step で計算されているものと同じ
        E_r_inv = self.beta_r / (self.alpha_r - 1.0).clamp(min=EPSILON)
        var_f = E_r_inv * torch.diag(self.S_f)
        E_sq_err = (self.y_full.squeeze() - self.m_f.squeeze())**2 + var_f
        
        # M-step 更新式
        new_noisescale = torch.mean(E_lambda * E_sq_err)
        self.noisescale.copy_(new_noisescale.clamp(min=EPSILON*100))
    ...
    def _m_step(self, optimizer, loss):
        ...
        loss.backward()
        optimizer.step()
        self._update_noisescale(local_params=(self.alpha_r, self.beta_r))
...
class SparseTPR(nn.Module):
    ...
    # 代入用の関数例
    def _update_noisescale_ca(self, local_params, y_batch, mu_f_batch, var_f_batch, var_lr):
        """
        座標上昇法(CA)によるノイズスケールの更新
        """
        with torch.no_grad():
            _, _, E_lambda_batch = local_params
            
            # 1. 期待二乗誤差 E_q[(y - f)^2] = (y - E[f])^2 + Var(f)
            E_sq_err_batch = (y_batch.squeeze() - mu_f_batch.squeeze())**2 + var_f_batch
            
            # 2. このバッチにおける推定値: (1/|B|) * sum( E[lambda] * E[sq_err] )
            # これが導出した sigma^2 のバッチ推定
            batch_noisescale = torch.mean(E_lambda_batch * E_sq_err_batch)
            
            # 3. 指数移動平均(SVIの更新ルール)でグローバルパラメータを更新
            # 既存の noisescale は exp(log_noisescale) なので、そのスケールで計算
            current_noisescale = torch.exp(self.log_noisescale)
            new_noisescale = (1 - var_lr) * current_noisescale + var_lr * batch_noisescale
            
            # 値を log_noisescale に戻して保存
            self.log_noisescale.data = torch.log(new_noisescale.clamp(min=EPSILON*100))
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for X_batch, y_batch in dataloader:
                ...
                self._update_noisescale_ca(local_params, X_batch, y_batch, K_XZ_batch, K_ZZ, var_lr)
