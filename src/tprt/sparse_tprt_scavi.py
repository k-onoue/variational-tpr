...
#         K_ZZ = self.kernel(self.Z, self.Z, params['lengthscale'], params['variance']) + torch.eye(self.M, device=self.Z.device) * jitter_val
...
#             L_precision_inner = torch.linalg.cholesky(precision_inner + torch.eye(self.M, device=K_ZZ.device) * jitter_val)
...
#             S_local_inv = torch.linalg.inv(S_u_local + torch.eye(self.M, dtype=S_u_local.dtype, device=S_u_local.device) * jitter_val)
...
#         L_S = torch.linalg.cholesky(self.S_u + torch.eye(self.M, dtype=self.S_u.dtype, device=self.S_u.device) * jitter_val)
...
#                 S_inv = torch.linalg.inv(self.S_u + torch.eye(self.M, dtype=self.S_u.dtype, device=self.S_u.device) * jitter_val)
...
#                 S_u_updated = torch.linalg.inv(-2.0 * eta_u2_updated + torch.eye(self.M, dtype=self.S_u.dtype, device=self.S_u.device) * jitter_val)
...
# # --- MODIFICATION START ---
# # Import the necessary components from the linear-operator library
...
#     #         Z_init = self.X_full[indices].clone()
#     #     return Z_init.to(dtype=self.X_full.dtype, device=self.device)
#     def _initialize_inducing_points(self, method="sobol"):
#         if self.N >= self.M:
#             if method == "kmeans":
#                 X_np = self.X_full.cpu().numpy()
#                 kmeans = KMeans(n_clusters=self.M, random_state=0, n_init='auto').fit(X_np)
...
#                     dimension=self.D, scramble=True
#                 )
#                 sobol_points_unit = sobol_engine.draw(n=self.M).to(self.device)
#                 Z_init = X_min + (X_max - X_min) * sobol_points_unit

#             elif method == "random":
#                 indices = np.random.choice(self.N, self.M, replace=False) # replace=Falseが一般的
#                 Z_init = self.X_full[indices].clone()
#             else:
...
#     def _compute_common_terms(self, X_batch, params):
#         # Create a LinearOperator for the kernel matrix K_ZZ
#         K_ZZ_base = to_linear_operator(self.kernel(self.Z, self.Z, params['lengthscale'], params['variance']))

#         # Create the jitter diagonal tensor directly using the operator's shape and device.
#         diag_shape = K_ZZ_base.shape[-1]
#         jitter_diag = torch.full((diag_shape,), jitter_val, device=K_ZZ_base.device, dtype=K_ZZ_base.dtype)

#         # --- THIS BLOCK IS CORRECTED ---
#         # Wrap the jitter tensor in a DiagLinearOperator before adding it.
#         jitter_op = DiagLinearOperator(jitter_diag)
#         K_ZZ = AddedDiagLinearOperator(K_ZZ_base, jitter_op)
#         # --- END CORRECTION ---

#         # Perform Cholesky on the LinearOperator; L_ZZ is also a (Triangular)LinearOperator
#         L_ZZ = K_ZZ.cholesky()

#         K_XZ_batch = self.kernel(X_batch, self.Z, params['lengthscale'], params['variance'])

#         # Use the .solve() method of the LinearOperator
#         KXZ_KZZ_inv = K_ZZ.solve(K_XZ_batch.T).T

#         # Return the LinearOperator objects
#         return K_ZZ, L_ZZ, K_XZ_batch, KXZ_KZZ_inv
...
#             # Replace cholesky_solve with the solve method of the Cholesky factor operator
#             KZZ_inv_m_u = L_ZZ.solve(self.m_u)
...
#             # K_ZZ is an operator, B is a tensor. Must convert K_ZZ to dense for addition.
#             precision_inner = expected_r * K_ZZ.to_dense() + B
            
#             L_precision_inner = torch.linalg.cholesky(precision_inner + torch.eye(self.M, device=K_ZZ.device) * jitter_val)

#             # K_ZZ is an operator. Need to convert to dense for the solve operation with L_precision_inner
#             K_ZZ_dense = K_ZZ.to_dense()
#             S_u_local = K_ZZ_dense @ torch.cholesky_solve(K_ZZ_dense, L_precision_inner)
            
#             y_term = K_XZ_batch.T @ (y_batch.squeeze() * c)
#             m_u_local = K_ZZ_dense @ torch.cholesky_solve(y_term.unsqueeze(1), L_precision_inner)
            
#             # Replace cholesky_solve with the solve method
#             trace_term = torch.trace(L_ZZ.solve(S_u_local))
#             KZZ_inv_m_u_local = L_ZZ.solve(m_u_local)
...
#         # Replace manual logdet calculation with the operator's method
#         logdet_K_ZZ = K_ZZ.logdet()

#         # Replace cholesky_solve with the operator's solve method
#         trace_KZZinv_Su = torch.trace(L_ZZ.solve(self.S_u))
...
#             self.log_nu_f, self.log_nu_epsilon, self.log_sigma_sq,
#             self.log_kernel_lengthscale, self.log_kernel_variance, self.Z,
#             # self.m_u, self.S_u, self.alpha_r, self.beta_r
#         ]
#         optimizer = optim.Adam(parameters_to_optimize, lr=lr)
#         dataset = TensorDataset(self.X_full, self.y_full)
#         dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
...
#                 # --- MODIFICATION START: Sequential update of global variational parameters ---
...
#                 # 2. Update q(u) based on the newly updated q(r)
#                 # Re-calculate local parameters for u using the updated r
#                 with torch.no_grad():
#                     new_expected_r = self.alpha_r / self.beta_r

#                     # Re-calculate intermediate terms that depend on r
#                     expected_lambda = alpha_lambda.squeeze() / beta_lambda.squeeze()
#                     c = expected_lambda / params['sigma_sq']

#                     K_ZZ, _, K_XZ_batch, _ = common_terms
#                     B = (K_XZ_batch.T * c) @ K_XZ_batch
                    
#                     precision_inner_new = new_expected_r * K_ZZ.to_dense() + B
#                     L_precision_inner_new = torch.linalg.cholesky(precision_inner_new + torch.eye(self.M, device=K_ZZ.device) * jitter_val)

#                     K_ZZ_dense = K_ZZ.to_dense()
#                     S_u_local_new = K_ZZ_dense @ torch.cholesky_solve(K_ZZ_dense, L_precision_inner_new)
                    
#                     y_term = K_XZ_batch.T @ (y_batch.squeeze() * c)
#                     m_u_local_new = K_ZZ_dense @ torch.cholesky_solve(y_term.unsqueeze(1), L_precision_inner_new)
                    
#                     S_local_inv_new = torch.linalg.inv(S_u_local_new + torch.eye(self.M, dtype=S_u_local_new.dtype, device=S_u_local_new.device) * jitter_val)
#                     eta_u1_local_new = S_local_inv_new @ m_u_local_new
#                     eta_u2_local_new = -0.5 * S_local_inv_new
                
#                 # Perform the global update for u using the re-calculated local parameters
#                 S_inv_global = torch.linalg.inv(self.S_u + torch.eye(self.M, dtype=self.S_u.dtype, device=self.S_u.device) * jitter_val)
#                 eta_u1_global = S_inv_global @ self.m_u
#                 eta_u2_global = -0.5 * S_inv_global

#                 eta_u1_updated = (1 - rho) * eta_u1_global + rho * eta_u1_local_new * scale
#                 eta_u2_updated = (1 - rho) * eta_u2_global + rho * eta_u2_local_new * scale
...
class SparseTPRTMiniBatch(nn.Module):
    ...
    # --- MODIFICATION START: Implement iterative refinement in E-Step ---
    def _e_step(self, X_batch, y_batch, common_terms, max_iters=1, tol=1e-5):
        with torch.no_grad():
            params = self._get_hyperparams()
            K_ZZ, L_ZZ, K_XZ_batch, KXZ_KZZ_inv = common_terms
            k_ii_batch = params['variance'].expand(X_batch.shape[0])

            # 1. Initialize local expectations from global parameters
            expected_r = self.alpha_r / self.beta_r
            expected_lambda = torch.ones(X_batch.shape[0], device=self.device)
            m_u_local = self.m_u.clone()
            S_u_local = self.S_u.clone()

            # 2. Start the refinement loop (Coordinate Ascent)
            for i in range(max_iters):
                m_u_old = m_u_local.clone()

                # A. Update q(lambda) using current E[u] and E[r]
                expected_r_inv = self.beta_r / (self.alpha_r - 1.0) if self.alpha_r > 1 else self.beta_r
                KZZ_inv_m_u = L_ZZ.solve(m_u_local)
                expected_f_mean = K_XZ_batch @ KZZ_inv_m_u
                var_f_term1 = expected_r_inv * (k_ii_batch - (KXZ_KZZ_inv * K_XZ_batch).sum(dim=1))
                var_f_term2 = (KXZ_KZZ_inv @ S_u_local @ KXZ_KZZ_inv.T).diag()
                var_f = (var_f_term1 + var_f_term2).unsqueeze(1)
                ...
                # B. Update q(r) using current E[u]
                trace_term = torch.trace(L_ZZ.solve(S_u_local))
                mean_term = m_u_local.T @ L_ZZ.solve(m_u_local)
                expected_u_quadratic = trace_term + mean_term
                alpha_r_local = params['nu_f'] / 2.0 + self.M / 2.0
                beta_r_local_val = params['nu_f'] / 2.0 + 0.5 * expected_u_quadratic.squeeze()
                expected_r = alpha_r_local / beta_r_local_val

                # C. Update q(u) using updated E[lambda] and E[r]
                c = expected_lambda / params['sigma_sq']
                B = (K_XZ_batch.T * c) @ K_XZ_batch
                precision_inner = expected_r * K_ZZ.to_dense() + B
                L_precision_inner = torch.linalg.cholesky(precision_inner + torch.eye(self.M, device=K_ZZ.device) * jitter_val)
                ...
                # 3. Convergence Check
                change = torch.norm(m_u_local - m_u_old) / (torch.norm(m_u_old) + 1e-9)
                if change < tol:
                    print(f"CA converged at step {i+1}!")
                    break

            # 4. Convert final converged local params to natural parameters
            S_local_inv = torch.linalg.inv(S_u_local + torch.eye(self.M, dtype=S_u_local.dtype, device=S_u_local.device) * jitter_val)
            eta_u1_local = S_local_inv @ m_u_local
            eta_u2_local = -0.5 * S_local_inv
            eta_r1_local = alpha_r_local - 1.0
    ...
    def fit(self, epochs=100, batch_size=64, lr=0.01, 
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        elbo_history = []
        if X_test is not None and y_test is not None and result_path:
            result_path = Path(result_path)
            result_path.parent.mkdir(parents=True, exist_ok=True)
            with open(result_path, 'w') as f: f.write("epoch,rmse,elbo\n")
        print(f"Starting Variational EM optimization for {epochs} epochs...")
        for epoch in range(epochs):
            ...
            for batch_idx, (X_batch, y_batch) in enumerate(dataloader):
                ...
                # The new E-step now handles all the local refinement internally
                eta_u1_local, eta_u2_local, eta_r1_local, eta_r2_local, alpha_lambda, beta_lambda = self._e_step(X_batch, y_batch, common_terms)
                ...
                # --- MODIFICATION START: Reverted to simpler, parallel global update ---
                # No re-evaluation is needed as _e_step now returns converged local parameters
                S_inv = torch.linalg.inv(self.S_u + torch.eye(self.M, dtype=self.S_u.dtype, device=self.S_u.device) * jitter_val)
                eta_u1_global = S_inv @ self.m_u
                eta_u2_global = -0.5 * S_inv
                eta_r1_global = self.alpha_r - 1.0
                ...
                # Update all global natural parameters in parallel
                eta_u1_updated = (1 - rho) * eta_u1_global + rho * eta_u1_local * scale
                eta_u2_updated = (1 - rho) * eta_u2_global + rho * eta_u2_local * scale
                eta_r1_updated = (1 - rho) * eta_r1_global + rho * eta_r1_local * scale
                eta_r2_updated = (1 - rho) * eta_r2_global + rho * eta_r2_local * scale
    ...
    def evaluate_model(self, epochs=100, batch_size=64, lr=0.01,
                       base_lr=1.0, kappa_e=1.0, tau_e=1.0, tau_b=1.0,
                       X_test=None, y_test=None, eval_interval=10,
                       result_path=None):
        ...
        if X_test is not None and y_test is not None and result_path is not None:
            result_path = Path(result_path)
            if not result_path.exists():
                result_path.parent.mkdir(parents=True, exist_ok=True)
                with open(result_path, 'w') as f:
